...
class SetupError(GoldAIError):
    """Raised when environment or directory setup fails."""
...
class DataPrepError(GoldAIError):
    """Raised when prepare_datetime or related steps fail."""
...
def setup_output_directory(base_dir: str, dir_name: str) -> str:
    ...
    setup_dir_logger.info("   (Setup) Preparing output directory...")
    try:
        os.makedirs(output_path, exist_ok=True)
        setup_dir_logger.info("      -> Directory exists or was created.")
        test_file_path = os.path.join(output_path, ".write_test")
        with open(test_file_path, "w", encoding="utf-8") as f:
            f.write("test")
        os.remove(test_file_path)
        setup_dir_logger.info("      -> Write permission confirmed.")
        return output_path
    except OSError as e:
        setup_dir_logger.error("   (Error) Unable to create or write to the directory")
        raise SetupError("Unable to create or write to the directory") from e
    except Exception as e:
        setup_dir_logger.error("   (Error) Unexpected error while preparing output directory")
        raise SetupError("Unexpected error while preparing output directory") from e
...
def prepare_datetime(df_pd: pd.DataFrame, timeframe_str: str = "", config: Optional['StrategyConfig'] = None) -> pd.DataFrame: # type: ignore
    ...
    try:
        if "Date" not in df_pd.columns or "Timestamp" not in df_pd.columns:
                prep_dt_logger.critical("(Error) Missing Date or Timestamp columns")
                raise DataPrepError("Missing Date or Timestamp columns")
